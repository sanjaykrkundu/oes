...
function login() {
	...
	$("#loginForm").submit(
			function(event) {
				...
				var dobgettime = dob.getTime()
						+ (dob.getTimezoneOffset() * 60000);
...
function loadUserData() {
	...
	window.onbeforeunload = function() {
		return "You Will Loose All Your Data. Don't Reload This Page.";
	}
	...
	$("#data").load("userdata.html", function() {
		...
		$.getJSON("./data/question.json", function(result) {
			...
			_mintime = (_time - parseInt(result["mintime"])) * 60;
...
function examSetup() {
	...
	for (var i = 1; i <= _questionno; i++)
		$("#questionNoBox").append(
				'<div class="col-xs-2 "><button class="btn btn-default qid" id="q'
						+ i + '">' + i + ' </button></div>');
	...
	$("#btnNext").click(function() {
		questionClicked("save", _currentQuestion + 1);
	});
	$("#btnReview").click(function() {
		questionClicked("review", _currentQuestion + 1);
	});
...
function questionClicked(mode, newCurrentQuestion) {
	var val = $('input[name="userAns"]:checked').val();
	$("#q" + (_currentQuestion + 1)).css("color", "#fff").css("font-weight",
			"normal").css("background-color", "");
	if (val) {
	/*	if (mode == "save")
			$("#q" + (_currentQuestion + 1)).removeClass("btn-default")
...
function loadQuestion(i) {
	...
	$("#q" + (i + 1)).css("color", "green").css("font-weight", "bold").css(
...
function countTimer() {
	...
	$('#time').text(
			formatString(Math.floor(__time / 60)) + ":"
					+ formatString(__time % 60));
}
	...
	$.when().done(
			function() {
				...
				var resultData = '{\n"name": "' + _name + '",\n"dob":"' + _dob
						...
						+ _questionno + '",\n"attended":"' + _attended
						+ '",\n"correct":"' + _correct + '"\n}';
				saveResult(resultData, _course.toUpperCase() + "_"
						+ _name.toLowerCase() + "_" + _dob.getTime());
				endExam();
			});
}
